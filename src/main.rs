...
impl Context {
    ...
    fn do_release(&mut self) -> Result<(), Error> {
        ...
        if build_manifest.exists() {
            ...
            // Generate checksums for all the downloaded files that weren't pruned earlier. This
            // does *not* include the manifests, as those are in a different directory.
            signer.sign_directory(&self.dl_dir())?;

            // Generate checksums for all the files generated by build-manifest.
            signer.sign_directory(&self.manifest_dir())?;

            // Merge the manifests directory into the downloads directory.
